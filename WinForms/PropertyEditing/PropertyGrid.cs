...
namespace AdamsLair.WinForms.PropertyEditing
{
	...
	public partial class PropertyGrid : UserControl
	{
		...
		public void SelectObjects1(IEnumerable<object> objEnum, bool readOnly = false, int scheduleMs = 0)
		{
			...
			foreach (object o in objEnum)
			{
				...
				for (int i = 0; i < attributes.Length; i++)
				{
					...
					{ 
						PropertyGridAttribute a = (PropertyGridAttribute) attributes[i];
						if (a.Ignore == false)
							this.selectedObjects.Add ( o );
						else
						{
							MessageBox.Show ( "Ignoring " + a.ToString (), "Ignoring" );
						}
					}
				}
			}
		...
		public void SelectObjects ( IEnumerable<object> objEnum, bool readOnly = false, int scheduleMs = 0 )
		{
			...
			foreach (object o in objEnum)
			{
				if (!(typeof ( IEnumerable ).IsAssignableFrom ( o.GetType () )) || (o.GetType () == typeof ( string )))
				{
				...
				else //object is Enumerable
				{
					//http://stackoverflow.com/questions/721441/c-sharp-how-to-iterate-through-classes-fields-and-set-properties

					foreach (object ob in objEnum)
					{
						SelectObjects ( new[] { ob }, readOnly, scheduleMs );
					}
				}
			}
		...
		public void SelectObjects3 ( IEnumerable<object> objEnum, bool readOnly = false, int scheduleMs = 0 )
		{
			this.selectedObjects.Clear ();
			...
			foreach (object o in objEnum)
			{
				...
				{
					SelectObjects ( new[] {o}, readOnly, scheduleMs );
				}
			}
		...
		public void SelectObject2 ( IEnumerable<object> objEnum, bool readOnly = false, int scheduleMs = 0 )
		{
			if (!(typeof ( IEnumerable ).IsAssignableFrom ( objEnum.GetType () )) || (objEnum.GetType () == typeof ( string )))
			{
				//replace with conditional Add()
				System.Reflection.MemberInfo info = objEnum.GetType ();
				object[] attributes = info.GetCustomAttributes ( true );
				for (int i = 0; i < attributes.Length; i++)
				{
					if (attributes[i] is PropertyGridAttribute)
					{
						PropertyGridAttribute a = (PropertyGridAttribute)attributes[i];
						if (a.Ignore == false)
							this.selectedObjects.Add ( objEnum );
						else
						{
							MessageBox.Show ( "Ignoring " + a.ToString (), "Ignoring" );
						}
